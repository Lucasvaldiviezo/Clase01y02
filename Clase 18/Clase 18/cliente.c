...
static int isValidId(char* clienteId);

Cliente* cliente_new(void)
{
    return (Cliente*) malloc(sizeof(Cliente));
}
Cliente* cliente_newConParametros(char* nombre, char* apellido,char* clienteId)
{
    Cliente* p;
    p=cliente_new();
    if(p!=NULL)
...
int cliente_setId(Cliente* this,char* idCliente)
{
    int retorno=-1;
    //validar!!
    if(this != NULL && isValidId(idCliente)==0)
    {
        this->clienteId=atoi(idCliente);
        retorno=0;
    }
    return retorno;
}
...
static int isValidId(char* clienteId)
{
    ...
    while(clienteId[i] != '\0')
    {
        if(clienteId[i] < '0' || clienteId[i] > '9')
        {
            retorno=-1;
            break;
        }
